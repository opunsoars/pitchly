...
def initialise_players(team, teamname, params):
    ...
    """
    # get player  ids
    player_ids = np.unique([c.split('_')[1]
                            for c in team.keys() if c[:4] == teamname])
    # create list
    team_players = []
    for p in player_ids:
...
class player(object):
    ...
    def probability_intercept_ball(self, T):
        ...
        f = 1/(1. + np.exp(-np.pi/np.sqrt(3.0) /
                           self.tti_sigma * (T-self.time_to_intercept)))
        return f
...
def default_model_params(time_to_control_veto=3):
    ...
    params['time_to_control_att'] = time_to_control_veto * \
        np.log(10) * (np.sqrt(3) *
                      params['tti_sigma']/np.pi + 1/params['lambda_att'])
    params['time_to_control_def'] = time_to_control_veto * \
        np.log(10) * (np.sqrt(3) *
                      params['tti_sigma']/np.pi + 1/params['lambda_def'])
    return params
...
def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params, field_dimen=(106., 68.,), n_grid_cells_x=50):
    ...
    assert 1 - \
        checksum < params['model_converge_tol'], "Checksum failed: %1.3f" % (
            1-checksum)
    return PPCFa, xgrid, ygrid


def calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params, return_individual=False):
    ...
    tau_min_att = np.nanmin([p.simple_time_to_intercept(
        target_position) for p in attacking_players])
    tau_min_def = np.nanmin([p.simple_time_to_intercept(
        target_position) for p in defending_players])
        ...
        while 1-ptot > params['model_converge_tol'] and i < dT_array.size:
            ...
            for player in attacking_players:
                ...
                dPPCFdT = (1-PPCFatt[i-1]-PPCFdef[i-1]) * \
                    player.probability_intercept_ball(T) * params['lambda_att']
            ...
            for player in defending_players:
                # calculate ball control probablity for 'player' in time interval T+dt
                dPPCFdT = (1-PPCFatt[i-1]-PPCFdef[i-1]) * \
                    player.probability_intercept_ball(T) * params['lambda_def']
                # make sure it's greater than zero
...
def generate_pitch_control_for_frame(hometeam, awayteam, frame, params, attacking, field_dimen=(106., 68.,), n_grid_cells_x=50, return_individual=False):
    ...
    for i in range(len(ygrid)):
        for j in range(len(xgrid)):
            target_position = np.array([xgrid[j], ygrid[i]])
            if return_individual == True:
                PPCFa[i, j], PPCFd[i, j], Patt, Pdef = calculate_pitch_control_at_target(target_position,
...
def get_pitch_control(frame_data)
    ...
    pitch_control_dict = {f:{} for f in hometeam.index}
    fails = []
    for frame in tqdm_notebook(hometeam.index):
        try:
            PPCFa, xgrid, ygrid, PPCFa_pax = generate_pitch_control_for_frame(hometeam, awayteam, frame, params, attacking='Home',return_individual=True)  
            pitch_control_dict[frame]['PPCFa'] = PPCFa
            pitch_control_dict[frame]['xgrid'] = xgrid    
            pitch_control_dict[frame]['ygrid'] = ygrid    
            pitch_control_dict[frame]['PPCFa_pax'] = PPCFa_pax   
        except:
            fails.append(frame)
